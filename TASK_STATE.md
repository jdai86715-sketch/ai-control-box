...
- Qwen 的工具调用提示已改为按候选集分开注入：无候选工具时只发送自然对话基础提示，不会复读 `native tool_call`；有候选工具时，基础提示与原生调用提示合并为一个 system message，避免 llama-server 对多个 system message 的不稳定处理。已真实验证中文工具清单、当前时间与英文 `tool list` 均返回原生调用。
...
- 用户重启服务后在 WebUI 验收指定城市的当前、逐小时天气两步调用；默认地点的自然语言天气触发仍需单独观察，不把 Qwen 3B 的当前表现当作已解决。
