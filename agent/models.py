...
from .tools import candidate_tool_context, get_registry, tool_schemas
...
class ModelRouter:
    ...
    def _ensure_current(self) -> None:
        ...
        if self._model is None or self._tool_version != registry.version or self._active_model != settings["active_model"]:
            self._model = NeedleModel(tool_schemas(), str(TOOL_INDEX_PATH)) if settings["active_model"] == "needle" else QwenModel(self._runtime, settings["qwen"]["model_id"], candidate_tool_context)
